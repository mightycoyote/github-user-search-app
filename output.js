...
export function displayResponse(response) {
    ...
    <div class="avatar">
      <img src="${response.avatar_url}" alt="avatar" id="avatar">
    </div>

    <div class="intro">
      <div>
        <h2>${response.name || response.login}</h2>
        <p class="login">@${response.login}</p>
      </div>
      <div>
    ...
    <div class="bio">
      <p>${response.bio || `<span class="not-available">This profile has no bio</span>`}</p>
    </div>

    <div class="stats-box">
      <div>
        <div class="tiny-heading">Repos</div>
        <div class="bold-number">${response.public_repos}</div>
      </div>
      <div>
        <div class="tiny-heading">Followers</div>
        <div class="bold-number">${response.followers}</div>
      </div>
      <div>
        <div class="tiny-heading">Following</div>
        <div class="bold-number">${response.following}</div>
      </div>
    </div>

    <div class="info-columns">

      <div class="other-info ${response.location === null ? `not-available` : ''}">
        ...
        <p class="other-info-text">${response.location || `<span>Not Available</span>`}</p>
      </div>

      <div class="other-info ${response.blog === null ? `not-available` : ''}">
        ...
        <p class="other-info-text">${response.blog === null ? `<span>Not Available</span>` : `<a href="${response.blog}">${response.blog}</a>`}</p>
      </div>

      <div class="other-info ${response.twitter_username === null ? `not-available` : ''}">
        ...
        <p class="other-info-text">${response.twitter_username === null ? `<span>Not Available</span>` : `<a href="https://twitter.com/${response.twitter_username}">${response.twitter_username}</a>`}</p>
      </div>
      
      <div class="other-info ${response.company === null ? `not-available` : ''}">
        <svg role="img" height="20" width="20" xmlns="http://www.w3.org/2000/svg">
          <title>company</title>
          <g class="icon" fill="#4b6a9b">
            <path
        ...
        <p class="other-info-text">${response.company === null ? `<span>Not Available</span>` : `<a href="https://github.com/${response.company.replace("@" , "")}">${response.company}</a>`}</p>
      </div>
    </div>
